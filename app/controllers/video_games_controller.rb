class VideoGamesController < ApplicationController
  ...
  def create
    title = params[:video_game][:title]
    description = params[:video_game][:description]
    # analyses = params[:video_game][:analyses]
    device = params[:video_game][:device]
    editor = params[:video_game][:editor]

    video_game = VideoGame.create({title: title, description:description, device:device, editor:editor})
  ...
  private
    ...
    def video_game_params
      params.require(:video_game).permit(:title, :description, :critic, :device, :editor)
    end
end
